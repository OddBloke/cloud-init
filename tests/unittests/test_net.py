...
class TestCmdlineDracutNetworkConfigSource(FilesystemMockingTestCase):

    def test_inapplicable_if_sysconfig_dir_doesnt_exist(self):
        # Switch to a temporary root to ensure we aren't picking up host files
        root = self.tmp_dir()
        self.reRoot(root)

        src = cmdline.DracutNetworkConfigSource()
        self.assertFalse(src.is_applicable())

    def test_inapplicable_if_only_dir_exists(self):
        # Switch to a temporary root to ensure we aren't picking up host files
        root = self.tmp_dir()
        os.makedirs(os.path.join(root, cmdline._DRACUT_SYSCONFIG_DIR_PATH.lstrip('/')))
        self.reRoot(root)

        src = cmdline.DracutNetworkConfigSource()
        self.assertFalse(src.is_applicable())

    def test_applicable_if_config_files_exist(self):
        # Switch to a temporary root to ensure we aren't picking up host files
        root = self.tmp_dir()
        # Just write an empty file
        ens3_path = os.path.join(
            cmdline._DRACUT_SYSCONFIG_DIR_PATH, "ifcfg-ens3"
        )
        populate_dir(root, {ens3_path: ''})
        self.reRoot(root)

        src = cmdline.DracutNetworkConfigSource()
        self.assertTrue(src.is_applicable())

    def test_render_config_one_dhcp_interface(self):
        ens3 = {
            "name": "ens3",
            "type": "physical",
            "subnets": [{"control": "manual", "type": "dhcp"}],
        }

        # Switch to a temporary root to ensure we aren't picking up host files
        root = self.tmp_dir()
        # Write out our example config
        ens3_path = os.path.join(
            cmdline._DRACUT_SYSCONFIG_DIR_PATH, "ifcfg-ens3"
        )
        populate_dir(root, {ens3_path: EXAMPLE_DRACUT_SYSCONFIG})
        self.reRoot(root)
        self.patchOpen(root)

        src = cmdline.DracutNetworkConfigSource()
        config = src.render_config()
        self.assertEqual(1, config['version'])
        self.assertEqual([ens3], config['config'])


class TestCmdlineConfigFromSysconfigFiles(CiTestCase):

    def test_no_files_returns_valid_but_empty_config(self):
        ret = cmdline.config_from_sysconfig_files([])
        self.assertEqual({'version': 1, 'config': []}, ret)

    @mock.patch('cloudinit.net.cmdline.config_entry_from_sysconfig_path')
    def test_files_are_used_to_generate_config(self, m_config_entry):
        # Use a simple lambda to check that things were passed through
        m_config_entry.side_effect = lambda path: ('config-from-' + path)
        ret = cmdline.config_from_sysconfig_files(['path1', 'path2'])
        expected_config = ['config-from-path1', 'config-from-path2']
        self.assertEqual({'version': 1, 'config': expected_config}, ret)

    @mock.patch('cloudinit.net.cmdline.config_entry_from_sysconfig_path')
    def test_an_empty_config_is_excluded(self, m_config_entry):
        # If we don't parse any config from a given path, we should exclude it
        # from the rendered config entirely
        m_config_entry.side_effect = ['config1', None, 'config3']
        ret = cmdline.config_from_sysconfig_files(['path1', 'path2', 'path3'])
        expected_config = ['config1', 'config3']
        self.assertEqual({'version': 1, 'config': expected_config}, ret)


class TestCmdlineConfigEntryFromSysconfigPath(FilesystemMockingTestCase):

    with_logs = True

    def _write_content_and_parse(self, content):
        root = self.tmp_dir()
        # Write out our example config
        path = os.path.join(
            cmdline._DRACUT_SYSCONFIG_DIR_PATH, "ifcfg-testing"
        )
        populate_dir(root, {path: content})
        self.reRoot(root)
        self.patchOpen(root)

        return path, cmdline.config_entry_from_sysconfig_path(path)

    def test_missing_type_returns_none_with_log_msg(self):
        content = EXAMPLE_DRACUT_SYSCONFIG.replace('TYPE=', 'OTHER=')
        config_path, config_entry = self._write_content_and_parse(content)
        self.assertIsNone(config_entry)
        self.assertIn(
            'No TYPE found in {}; skipping.'.format(config_path),
            self.logs.getvalue()
        )

    def test_missing_bootproto_returns_none_with_log_msg(self):
        content = EXAMPLE_DRACUT_SYSCONFIG.replace('BOOTPROTO=', 'OTHER=')
        config_path, config_entry = self._write_content_and_parse(content)
        self.assertIsNone(config_entry)
        self.assertIn(
            'No BOOTPROTO found in {}; skipping.'.format(config_path),
            self.logs.getvalue()
        )

    def test_missing_device_returns_none_with_log_msg(self):
        content = EXAMPLE_DRACUT_SYSCONFIG.replace('DEVICE=', 'OTHER=')
        config_path, config_entry = self._write_content_and_parse(content)
        self.assertIsNone(config_entry)
        self.assertIn(
            'No DEVICE found in {}; skipping.'.format(config_path),
            self.logs.getvalue()
        )

    def test_non_ethernet_returns_none_with_log_msg(self):
        wrong_type = 'Bond'
        content = EXAMPLE_DRACUT_SYSCONFIG.replace(
            'TYPE=Ethernet', 'TYPE={}'.format(wrong_type)
        )

        config_path, config_entry = self._write_content_and_parse(content)
        self.assertIsNone(config_entry)
        self.assertIn(
            'Unhandled TYPE ("{}") found in {}; skipping.'.format(
                wrong_type, config_path
            ),
            self.logs.getvalue()
        )

    def test_non_dhcp_returns_none_with_log_msg(self):
        wrong_bootproto = 'unknown'
        content = EXAMPLE_DRACUT_SYSCONFIG.replace(
            'BOOTPROTO=dhcp', 'BOOTPROTO={}'.format(wrong_bootproto)
        )

        config_path, config_entry = self._write_content_and_parse(content)
        self.assertIsNone(config_entry)
        self.assertIn(
            'Unhandled BOOTPROTO ("{}") found in {}; skipping.'.format(
                wrong_bootproto, config_path
            ),
            self.logs.getvalue()
        )

    def test_device_used_not_name(self):
        content = EXAMPLE_DRACUT_SYSCONFIG.replace(
            'BOOTPROTO=dhcp', 'BOOTPROTO={}'.format(wrong_bootproto)
        )
        self.skipTest('')
    ...
    def test_happy_path(self):
        _, config_entry = self._write_content_and_parse(
            EXAMPLE_DRACUT_SYSCONFIG
        )
        ...
        self.assertEqual(expected, config_entry)
